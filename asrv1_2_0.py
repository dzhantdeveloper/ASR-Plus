...
# Единый список команд, доступных покупателю (показывается по команде !команды)
BUYER_COMMANDS_TEXT = (
...
class LotConfig(BaseModel):
    ...
    # Сколько часов аренды даёт ОДНА купленная единица (штука) этого лота.
    # Например: hours_per_unit=1 -> 1 шт = 1 час (старое поведение "почасовых"
    # лотов); hours_per_unit=12 -> 1 шт = 12 часов, купили 2 шт = 24 часа и т.д.
    # Настраивается свободно в настройках лота, отдельно для каждого лота.
    ...
    # Отдельный, СОБСТВЕННЫЙ служебный #id лота-продления (легаси, одиночная
    # привязка extend_lot_id). Генерируется заново при каждой привязке и НЕ
    # совпадает с match_tag основного лота — чтобы разные физические лоты
    # никогда не несли один и тот же #id (источник бага "нет свободных
    # аккаунтов" из-за коллизии одинаковых ID).
...
class Settings(BaseModel):
    ...
    def get_lot(self, lot_id: str) -> Optional[LotConfig]:
        ...
        if isinstance(raw, dict):
            # Обратная совместимость: у старых конфигов был lot_type="fixed" +
            # fixed_hours — конвертируем их в hours_per_unit при чтении.
    ...
    def set_lot_hours_per_unit(self, lot_id: str, hours_per_unit: float):
        """Задать/изменить, сколько часов даёт 1 купленная штука этого лота."""
        existing = self.lots.get(str(lot_id), {})
        if isinstance(existing, str):
            existing = {"tag": _ntag(existing)}
        existing["hours_per_unit"] = float(hours_per_unit)
    ...
    def find_main_lot_by_configured_extend_id(self, raw_offer_id: str) -> Optional[str]:
        ...
        лот), а структурная привязка в настройках бота — нет."""
        if not raw_offer_id:
            return None
    ...
    def all_used_service_tags(self) -> set:
        """Собирает ВСЕ служебные #id, которые уже где-либо используются:
        match_tag основных лотов + все write_tag лотов-продлений (и
        индивидуальных, и общих по тегу). Используется для генерации нового
        #id, который гарантированно ни с чем не совпадёт."""
        used = set()
    ...
    def gen_new_service_tag(self) -> str:
        """Генерирует СОВЕРШЕННО НОВЫЙ, гарантированно уникальный служебный
        #id для лота-продления — никогда не совпадающий ни с #id основного
        лота, ни с любым другим уже выданным #id лота-продления."""
        return _gen_match_tag(self.all_used_service_tags())

    def set_lot_extend_write_tag(self, lot_id: str, write_tag: Optional[str]):
        """Сохраняет собственный #id (легаси, одиночная привязка extend_lot_id)."""
        raw = self.lots.get(str(lot_id), {})
        if isinstance(raw, str):
            raw = {"tag": _ntag(raw)}
    ...
    def set_lot_extend_option_write_tag(self, lot_id: str, ext_lot_id: str, write_tag: str):
        """Сохраняет собственный #id для конкретного варианта продления лота."""
        raw = self.lots.get(str(lot_id), {})
        if isinstance(raw, str):
            raw = {"tag": _ntag(raw)}
    ...
    def build_service_tag_map(self) -> Dict[str, str]:
        """Строит полную карту {служебный #id -> lot_id основного лота} —
        включает и match_tag основных лотов, и собственные write_tag всех
        лотов-продлений (индивидуальных и общих по тегу), которые теперь у
        КАЖДОГО лота-продления свои — совершенно новые, никогда не
        совпадающие ни с чьим ещё #id."""
        m: Dict[str, str] = {}
        for lid in self.lots:
    ...
    def build_extend_service_tags(self) -> set:
        """Возвращает множество СЛУЖЕБНЫХ #id, которые принадлежат именно
        лотам-продления (extend_write_tag / extend_options[].write_tag —
        и индивидуальным, и общим по тегу), в отличие от собственного
        match_tag основного лота. Нужно, чтобы при опознании заказа по #id
        (см. _match_lot_by_match_tag) можно было сразу и надёжно определить:
        это заказ пришёл именно С ЛОТА-ПРОДЛЕНИЯ (а не с основного лота,
        совпадение с match_tag которого тоже проходит через ту же карту) —
        не полагаясь при этом на сырой числовой offer_id заказа, который
        FunPay часто просто не отдаёт в событии/API (см. баг 'нет свободных
        аккаунтов' при оплате лота-продления)."""
        s = set()
        for lid in self.lots:
            cfg = self.get_lot(lid)
            if not cfg:
...
def _strip_match_tags_from_funpay_lot(c, funpay_lot_id: str) -> Tuple[bool, str]:
    """
    Удаляет из подробного описания лота funpay_lot_id ВСЕ служебные #id-теги
    (формата #id7k2p9a), которые бот когда-либо дописал туда как привязку
    лота-продления к основному лоту/тегу.

    Нужно вызывать при отвязке/замене лота-продления — иначе старый служебный
    #id так и остаётся навсегда в описании физического лота на FunPay, из-за
    чего заказ с этого давно отвязанного лота может ошибочно определиться как
    продление СТАРОГО основного лота (баг "старый лот не отвязывается").
    """
    try:
...
def _unlink_extend_lot(c, ext_lot_id: Optional[str]):
    """
    Полностью отвязывает лот-продление, который больше не используется
    (заменён другим лотом-продления при перепривязке):
    1) отменяет таймер автоотключения;
    2) выключает лот на FunPay через API;
    3) чистит из его описания старые служебные #id — чтобы он не оставался
       фантомно привязанным к старому основному лоту/тегу и не мог быть
       ошибочно куплен/опознан как чей-то ещё активный лот-продление.
    """
...
def _match_lot_by_match_tag_ex(description: str, order_id: Optional[str] = None) -> Tuple[Optional[str], bool]:
    """То же самое, что _match_lot_by_match_tag, но дополнительно возвращает
    флаг is_extend: True, если совпавший #id — это служебный write_tag
    лота-продления (а не собственный match_tag основного лота). Этот флаг
    нужен, чтобы надёжно понять "заказ пришёл именно с лота-продления",
    НЕ полагаясь на сырой числовой offer_id заказа — FunPay часто не отдаёт
    его ни в событии, ни в API-объекте заказа, из-за чего старая проверка
    (сравнение raw_offer_id с extend_lot_id) молча не срабатывала и заказ
    ошибочно уходил на выдачу нового аккаунта вместо продления аренды."""
    ...
    # Полная карта: и #id основных лотов, и СОБСТВЕННЫЕ (совершенно новые,
    # никогда не повторяющиеся) #id лотов-продления — и индивидуальных, и
    # общих по тегу.
...
def _find_lot_id_for_order_ex(c, event) -> Tuple[Optional[str], bool]:
    """
    Определяет lot_id для заказа. Дополнительно возвращает флаг is_extend:
    True, если заказ опознан именно как покупка С ЛОТА-ПРОДЛЕНИЯ (шаг -1
    структурной привязки, либо шаг 0 по служебному write_tag лота-продления),
    а не с основного лота. Раньше это приходилось определять заново, отдельно
    сравнивая сырой числовой offer_id заказа с extend_lot_id — но FunPay
    часто просто не отдаёт offer_id ни в событии, ни в API-объекте заказа,
    из-за чего проверка молча не срабатывала и оплата лота-продления
    ошибочно уходила на выдачу нового аккаунта ("нет свободных аккаунтов").
    Теперь флаг is_extend берётся напрямую из того же надёжного резолвинга,
    который уже нашёл lot_id — никакого отдельного сравнения не требуется.
    ...
    def _found(lot_id: str, is_extend: bool = False) -> Tuple[str, bool]:
        """Хук: вызывается когда lot_id найден. Возвращает (lot_id, is_extend) без изменений."""
        return lot_id, is_extend
    ...
    # (extend_lot_id / extend_options), ДО парсинга текстового
...
def _effective_extend_lot_id(lot_cfg: "LotConfig") -> Optional[str]:
    """Лот-продление (одиночный) для лота, если задан индивидуально."""
    if not lot_cfg:
        return None
    return lot_cfg.extend_lot_id

def _effective_extend_options(lot_cfg: "LotConfig") -> List[Dict[str, Any]]:
    """Индивидуальные варианты продления (время) для лота."""
    if not lot_cfg:
        return []
...
# Ожидание выбора КОНКРЕТНОГО АККАУНТА для продления, когда у покупателя
# 2 и более активных аренды одновременно — чтобы не продлить не тот аккаунт,
# сначала спрашиваем, какой именно он хочет продлить (по номеру из списка).
_extend_account_choice_pending: Dict[int, Dict[str, Any]] = {}
_extend_account_choice_lock = threading.Lock()
EXTEND_ACCOUNT_CHOICE_TIMEOUT = 300  # 5 минут на выбор аккаунта
...
def process_new_order(c, event):
    ...
    def _do_process():
        ...
        try:
            ...
            # Единая логика для ВСЕХ лотов: 1 купленная штука = hours_per_unit
            # часов аренды (настраивается отдельно для каждого лота, по
            # умолчанию 1ч = 1шт, как раньше у "почасовых" лотов).
            # Если оплата пришла с одного из лотов-продлений (лота или тега),
            # у которого задан СВОЙ курс часов-за-штуку — используем именно
            # его вместо курса основного лота, но количество купленных штук
            # всё равно учитывается (12ч=1шт, купили 2шт -> 24ч).
            ...
            with _data_lock:
                ...
                if existing:
                    # ГЛАВНАЯ, надёжная проверка: _find_lot_id_for_order_ex уже
                    # сообщает напрямую, было ли совпадение с #id именно
                    # ЛОТА-ПРОДЛЕНИЯ (структурная привязка extend_lot_id на шаге -1,
                    # либо служебный write_tag на шаге 0) — а не с match_tag обычного
                    # лота. Раньше это приходилось определять заново, сравнивая сырой
                    # числовой offer_id заказа (raw_offer_id) с extend_lot_id — но
                    # FunPay очень часто просто не отдаёт offer_id ни в событии, ни в
                    # API-объекте заказа (видно в логах: "offer_id=?, lot_id=?"), из-за
                    # чего raw_offer_id получался пустым, проверка молча не срабатывала,
                    # и оплата лота-продления ошибочно уходила на выдачу НОВОГО
                    # аккаунта — отсюда и "нет свободных аккаунтов" при возврате денег.
                    # Теперь используем флаг, полученный из того же надёжного резолва,
                    # который уже нашёл lot_id, — никакого отдельного offer_id не нужно.
                    if lot_id_is_extend:
                    ...
                    # Фолбэк (легаси): сравнение через raw_offer_id, если он всё же
                    # доступен и/или резолв почему-то не пометил заказ как extend.
                    if not is_extend_purchase and raw_offer_id:
                        if _ntag(raw_offer_id) != _ntag(lot_id) and existing.acc_id:
                            ...
                            if existing_acc and _ntag(existing_acc.tag) == _ntag(tag):
                                is_extend_purchase = True
                    # Фолбэк 2 (легаси): старая проверка через lot_cfg исходной покупки —
                    # оставлена на случай, если резолв lot_id почему-то не сработал.
                ...
                if existing and order_id not in ORDERS:
                    ...
                    if acc and acc.rental_end:
                        ...
                        # ВАЖНО (фикс бага): раньше при отсутствии raw_offer_id таймер
                        # пытались отменить по `lot_id`, а это ID ОСНОВНОГО лота (после
                        # резолва через тег), а не лота-продления, на который таймер
                        # реально был поставлен — ключи не совпадали, таймер оставался
                        # висеть и через 5 минут слал продавцу ложное "не оплатил",
                        # хотя оплата уже прошла. Теперь на всякий случай отменяем
                        # таймеры по ВСЕМ лотам-продлениям, настроенным для этого лота
                        # и тега (включая все варианты extend_options), а не только
                        # по одному предполагаемому ID.
            ...
            # Сообщение о "подготовке аккаунта" покупателю больше не отправляется
            # по запросу — покупатель сразу получает готовые данные аккаунта одним
            # сообщением (order_completed), без промежуточного "подождите...".
            # Настройка notification_preparing оставлена для обратной совместимости
            # (используется только в внутренних тг-логах продавца, если нужно).
...
def process_message(c, event):
    ...
    # Если у покупателя открыт выбор КОНКРЕТНОГО АККАУНТА для продления
    # (было 2+ активных аренды одновременно) — пытаемся разобрать его ответ
    # как номер из присланного списка, не дожидаясь совпадения с обычными командами.
    ...
    if acc_pending:
        ...
        if idx is not None and 0 <= idx < len(order_ids):
            ...
            # Число введено, но номер не из списка — сообщаем и не сбрасываем
            # ожидание, чтобы можно было ответить ещё раз в течение TTL.
...
def _process_buyer_command(c, event, msg, is_code, is_time, is_extend, is_stock, is_account, is_seller, is_help=False):
    ...
    if is_code:
        ...
        if len(targets) >= 2:
            # У покупателя несколько активных аккаунтов — сначала спрашиваем,
            # какой именно он хочет продлить (по номеру), чтобы не продлить не тот.
...
def _process_extend_for_target(c, msg, order, acc):
    """Логика продления для КОНКРЕТНОГО (уже однозначно определённого) аккаунта/заказа:
    проверка вариантов времени продления, выбор лота-продления и отправка ссылки.
    Вынесено в отдельную функцию, чтобы использовать как при единственном активном
    аккаунте у покупателя, так и после того, как он выбрал нужный из нескольких."""
    lot_cfg = SETTINGS.get_lot(order.lot_id) if order.lot_id else None

    # Если для лота настроено несколько вариантов продления с разным
    # временем — спрашиваем покупателя, какой он хочет.
...
def init(card: Cardinal):
    ...
    def open_lots(c):
        """Единый список всех настроенных лотов (без деления на типы)."""
    ...
    def lot_extend_lot_menu(c):
        """Меню управления лот-продлением для конкретного лота (единая система
        вариантов: у каждого варианта — свой лот-продление и свой курс
        часов-за-1-штуку, например 12ч=1шт, 10ч=1шт и т.д.)."""
        lid = _p(c)
        lc = SETTINGS.get_lot(lid)
        if not lc:
            return answer(c, "❌ Лот не найден", True)
    ...
    def _open_extopt_list(c, lid):
        """Список вариантов продления (разное время за 1 шт) для лота."""
        lc = SETTINGS.get_lot(lid)
        if not lc:
            return answer(c, "❌ Лот не найден", True)
    ...
    def _h_extend_lot_id(m):
        ...
        def _write_tag_bg(mid=m.chat.id, main_id=main_lot_id, ext_id=extend_id, prev_msg=msg_sent,
            ...
            # Если к этому лоту раньше был привязан ДРУГОЙ лот-продление —
            # отвязываем старый: выключаем на FunPay и чистим его описание,
            # иначе он остаётся висеть как будто всё ещё активный/привязанный
            # (баг: старый лот-продление не отвязывается и не пропадает из списка).
    ...
    def lot_tag(c):
        ...
        try:
            ...
            pass
        # Время аренды (часов за 1 шт) нужно указать один раз — эти данные
        # реально используются при выдаче аккаунта. По умолчанию можно
        # оставить 1 (старое поведение "1 шт = 1 час").
    ...
    def lot_hours_per_unit(c):
        """Открыть диалог изменения курса часов-за-1-штуку для лота."""
        lid = _p(c)
        lc = SETTINGS.get_lot(lid)
        if not lc:
            return answer(c, "❌ Лот не найден", True)
